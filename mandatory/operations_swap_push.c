...
void	push_stack(t_stacks *stacks, char to_id, t_list **operations)
{
	t_stack	*from_stack;
	t_stack	*to_stack;
	t_item	*transfer;
	...
	add_operation(stacks, "push", to_id, operations);
}
...
void	swap_stack(t_stacks *stacks, char id, int count, t_list **operations)
{
	...
	if (count == 1)
		add_operation(stacks, "swap", id, operations);
}

void	swap_both(t_stacks *stacks, t_list **operations)
{
	swap_stack(stacks, 'A', 2, operations);
	swap_stack(stacks, 'B', 2, operations);
