...
/*   Updated: 2025/12/10 20:38:21 by laveerka      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static t_list	*substitute_operations(t_list *operations, char *id_first, char *id_second)
{
	...
	save_op = operations;
	save_first = 0;
	while (save_op && ft_strncmp(save_op->content, id_first, ft_strlen(id_first)) == 0)
	{
		save_op = save_op->next;
		save_first++;
	}
	save_second = 0;
	while (save_op && ft_strncmp(save_op->content, id_second, ft_strlen(id_second)) == 0)
	{
		save_op = save_op->next;
		save_second++;
	}
	...
	while (operations && save_first && save_second)
	{
		if (save_first % 2 == 0)
		{
			if (ft_strncmp(id_first, "ra", ft_strlen("ra")) == 0 || !ft_strncmp(id_first, "rb", ft_strlen("rb")))
				ft_printf("rr\n");
			else if (ft_strncmp(id_first, "rra", ft_strlen("rra")) == 0 || !ft_strncmp(id_first, "rrb", ft_strlen("rrb")))
				ft_printf("rrr\n");
