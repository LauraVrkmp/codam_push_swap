...
/*   Updated: 2025/12/07 06:04:26 by laveerka      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	move_from_top_a(t_stacks *stacks, t_chunk chunk, t_list **operations)
{
	...
	{
		if (rotate_to_order(stacks, operations) || (section_sorted(stacks, stacks->a->first->rank, stacks->total, stacks->a->size - 1) && stacks->total - stacks->a->first->rank == chunk.high_max - chunk.low_min))
			return ;
		...
		else if (stacks->a->first->rank >= chunk.high_min && stacks->a->first->rank <= chunk.high_max && !section_sorted(stacks, stacks->a->first->rank, stacks->total, stacks->a->size - 1))
		{
			//ft_printf("from top A rotate A\n");
			rotate_stack(stacks, 'A', 1, operations);
			//print_stacks(stacks);
		}
		else if (stacks->a->first->rank >= chunk.low_min && stacks->a->first->rank <= chunk.mid_max && !section_sorted(stacks, stacks->a->first->rank, stacks->total, stacks->a->size - 1))
		{
...
void	move_from_top_b(t_stacks *stacks, t_chunk chunk, t_list **operations)
{
	...
	while (max_op < (chunk.high_max - chunk.low_min + 1) && stacks->b->size > 0)
	{
		...
		if (stacks->a->last->rank + 1 == stacks->a->last->prev->rank && stacks->a->last->rank + 2 == stacks->a->first->rank && section_sorted(stacks, stacks->a->first->rank, stacks->total, stacks->a->size - 1))
		{
			//ft_printf("from top B reverse rotate A twice and swap\n");
		...
		else if (stacks->b->first->rank + 2 == stacks->a->first->rank && stacks->b->first->next->rank + 1 == stacks->a->first->rank && section_sorted(stacks, stacks->a->first->rank, stacks->total, stacks->a->size))
		{
...
void	move_from_bottom_a(t_stacks *stacks, t_chunk chunk, t_list **operations)
{
	...
	while (stacks->a->last->rank >= chunk.low_min && stacks->a->last->rank <= chunk.high_max)
	{
		...
		reverse_rotate_stack(stacks, 'A', 1, operations);
		//print_stacks(stacks);
		if (stacks->a->first->rank - 1 == stacks->a->first->next->rank && section_sorted(stacks, stacks->a->first->next->next->rank, stacks->total, stacks->a->size))
		{
			//ft_printf("from bottom A swap\n");
			swap_stack(stacks, 'A', 1, operations);
			//print_stacks(stacks);
		}
		else if (stacks->a->first->rank + 1 == stacks->a->last->rank && stacks->a->last->rank + 1 == stacks->a->first->next->rank && section_sorted(stacks, stacks->a->first->next->rank, stacks->total, stacks->a->size - 1))
		{
		...
		else if (!section_sorted(stacks, stacks->a->first->rank, stacks->total, stacks->a->size - 1) && stacks->a->first->rank >= chunk.low_min && stacks->a->first->rank <= chunk.mid_max && chunk.low_min + 1 < chunk.high_max)
		{
...
void	move_from_bottom_b(t_stacks *stacks, t_chunk chunk, t_list **operations)
{
	...
	while (max_op < (chunk.high_max - chunk.low_min + 1) && stacks->b->last->rank >= chunk.low_min && stacks->b->last->rank <= chunk.high_max && stacks->b->size > 2)
	{
		...
		if (stacks->b->first->rank >= chunk.mid_min)
		{
			...
			push_stack(stacks, 'A', operations);
			//print_stacks(stacks);
			if (stacks->a->first->rank - 1 == stacks->a->first->next->rank && stacks->a->first->rank + 1 == stacks->a->first->next->next->rank && section_sorted(stacks, stacks->a->first->next->next->rank, stacks->total, stacks->a->size))
			{
