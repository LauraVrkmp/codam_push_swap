...
void	push_stack(t_stacks *stacks, char *count_id, t_list **operations)
{
	...
	if (count_id[1] == 'P')
		add_operation(stacks, "push", count_id[0], operations);
}
...
void	swap_stack(t_stacks *stacks, char to_id, char *count_id, t_list **operations)
{
	...
	if (count_id[0] == '1' && count_id[1] == 'P')
		add_operation(stacks, "swap", to_id, operations);
}

void	swap_both(t_stacks *stacks, char *count_id, t_list **operations)
{
	swap_stack(stacks, 'A', count_id, operations);
	swap_stack(stacks, 'B', count_id, operations);
	if (count_id[1] == 'P')
		add_operation(stacks, "swap both", 'b', operations);
}
